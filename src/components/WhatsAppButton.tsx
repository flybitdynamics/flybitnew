...
export default function WhatsAppButton() {
  ...
  const message = 'Hi, I want the enquiry for the drone show.';
  const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodeURIComponent(message)}`;
  const callNumber = 'tel:+919979850863';

  return (
    <>
      {/* Call Button - Placed above WhatsApp */}
      <a
        href={callNumber}
        className="fixed left-6 bottom-[4.8rem] z-50 flex items-center justify-center w-11 h-11 bg-gold text-black rounded-full shadow-lg hover:bg-gold-light hover:scale-110 transition-all duration-300 group md:cursor-none border border-gold/20"
        aria-label="Call FLYBIT Dynamics"
      >
        {/* Pulsing Outer Ring */}
      ...
      <a
        ...
        className="fixed left-6 bottom-6 z-50 flex items-center justify-center w-11 h-11 bg-[#25D366] text-white rounded-full shadow-lg hover:bg-[#20ba5a] hover:scale-110 transition-all duration-300 group md:cursor-none"
